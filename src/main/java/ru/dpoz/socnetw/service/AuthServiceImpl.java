...
public class AuthServiceImpl implements AuthService
{
    ...
    public boolean signUpBulk(List<SignUpInfo> signUpInfoList)
    {
        ...
        SocnetwApplication.logger.info("adding bulk users");
        List<UserEntity> users = new ArrayList<>();
        signUpInfoList.forEach(i -> users.add(i.getUser()));
        List<UUID> userIds = userDAO.addAll(users);
        Long t2 = System.currentTimeMillis();
        SocnetwApplication.logger.info("complete in {}", (t2 - t1)/1000F);

        t1 = System.currentTimeMillis();
        SocnetwApplication.logger.info("perform convert to hobbies list");
        ...
        t1 = System.currentTimeMillis();
        SocnetwApplication.logger.info("add bulk hobbies");
        userHobbiesDAO.addAll(hobbies);
        t2 = System.currentTimeMillis();
        SocnetwApplication.logger.info("complete in {}", (t2 - t1)/1000F);

        t1 = System.currentTimeMillis();
        SocnetwApplication.logger.info("add bulk secrets");
